...
trait ItTest extends BeforeAndAfterEach { self: Suite =>
  ...
  private val DatabaseConfig(username, password, jdbcUrl, name, port, _) = DbUrlParser.parse(config.db.databaseUrl).toOption.get

  private lazy val connection = DriverManager.getConnection(jdbcUrl, username, password)

  private lazy val container: Container = new FixedHostPortGenericContainer(
  ...
  abstract override def run(testName: Option[String], args: Args): Status = {
    val startContainer = Try(sys.env("IT_TEST_REUSE_DB")).isFailure
